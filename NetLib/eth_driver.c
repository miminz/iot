...
#include <rthw.h>
 ...
 __attribute__((__aligned__(4))) ETH_DMADESCTypeDef DMARxDscrTab[ETH_RXBUFNB];      /* MAC鎺ユ敹鎻忚堪绗� 锛�4瀛楄妭瀵归綈*/
 __attribute__((__aligned__(4))) ETH_DMADESCTypeDef DMATxDscrTab[ETH_TXBUFNB];      /* MAC鍙戦�佹弿杩扮锛�4瀛楄妭瀵归綈 */

 __attribute__((__aligned__(4))) uint8_t  MACRxBuf[ETH_RXBUFNB*ETH_RX_BUF_SZE];     /* MAC鎺ユ敹缂撳啿鍖猴紝4瀛楄妭瀵归綈 */
 __attribute__((__aligned__(4))) uint8_t  MACTxBuf[ETH_TXBUFNB*ETH_TX_BUF_SZE];     /* MAC鍙戦�佺紦鍐插尯锛�4瀛楄妭瀵归綈 */

__attribute__((__aligned__(4))) SOCK_INF SocketInf[WCHNET_MAX_SOCKET_NUM];          /* Socket淇℃伅琛紝4瀛楄妭瀵归綈 */
...
void ETH_PHYLink( void )
{
...
#else
    ETH_WritePHYRegister( gPHYAddress, 31,0x0a43 );
    phy_stat = ETH_ReadPHYRegister( gPHYAddress, 26);
    WCHNET_PhyStatus( phy_stat );
    if( phy_stat&(1<<2) )
    {
         /* PHY鍗忓晢缁撴灉 */
    ...
    phy_stat = ETH_ReadPHYRegister( gPHYAddress, 29);/* 璇籌NSR */
#endif
}
...
void EXT5_9_INT_INIT(void)
{
    ...
    EXTI_InitStructure.EXTI_Trigger = EXTI_Trigger_Falling;/* 涓嬮檷娌胯Е鍙戜腑鏂� */
    EXTI_InitStructure.EXTI_LineCmd = ENABLE;
    EXTI_Init(&EXTI_InitStructure);

    NVIC_InitStructure.NVIC_IRQChannel = EXTI9_5_IRQn;
    NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 1;/* 鎶㈠崰浼樺厛绾э紝娉ㄦ剰V4F鍐呮牳鐨勪腑鏂紭鍏堢骇 */
    NVIC_InitStructure.NVIC_IRQChannelSubPriority = 2;/* 瀛愪紭鍏堢骇 */
...
void RTL8211FS_interrupt_init(void)
{
    ...
    RegValue = ETH_ReadPHYRegister(1, 18);/* 璇籌NER */
    RegValue|=(1<<4)|(1<<3);/* 浣胯兘鐗╃悊灞傜姸鎬佹敼鍙樹腑鏂拰鍗忓晢瀹屾垚涓柇 */
